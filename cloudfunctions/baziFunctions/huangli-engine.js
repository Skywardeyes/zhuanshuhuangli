...
// ── 个性化宜忌过滤 ──
function filterActivities(baseYi, baseJi, userBazi, dayStem, dayBranch) {
  const dayWx = C.GAN_WUXING[dayStem];
  const dayMasterWx = C.GAN_WUXING[userBazi.dayGan];
  ...
  // 根据日柱冲合
  const clash = calcDayClash(dayBranch, userBazi.pillars);
  if (clash.dayClash) {
...
function generateDailyHuangli(dateStr, userBazi, baseHuangli) {
  ...
  // 冲合分析
  const clashResult = calcDayClash(dayBranch, userBazi.pillars);

  // 个性化宜忌
  const baseYi = base.yi || C.BASE_YI.slice(0, 10);
  const baseJi = base.ji || C.BASE_JI.slice(0, 8);
  const activities = filterActivities(baseYi, baseJi, userBazi, dayStem, dayBranch);
